...
2. 把安装包（`webknow-ai-<版本>-chrome.zip`）放进去，右键 → **全部解压缩**。
3. 解压后得到一个文件夹，里面能看到 `manifest.json` 和一个 `icon` 文件夹。
...
4. 接着它会让你确认：**这一页的文字会发给 DeepSeek**，费用从你的账号扣。请只在你确认这一页是公开的、你有权这样用的时候，点"我确认"。
5. 点"开始伴读"，等几秒，摘要和话题就出来了。
