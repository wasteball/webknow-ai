...
- **想让回答带上网上的资料**：先到"设置 → 联网搜索"启用一个搜索服务（支持 SearXNG、Tavily、博查，用你自己的账号或实例），问答输入框下面就会多一个"联网搜索"开关。它只把你的**搜索词**发给搜索服务，文章正文不会发给它；用了网络资料时，回答会标明"根据网络资料"并附上链接。
...
- **联网搜索开启时**：你的**搜索词**会发给你在设置里选的那个搜索服务（不是 DeepSeek，也不是我们），文章正文不会发给它。搜索服务怎么保存数据由它自己的规则决定。
