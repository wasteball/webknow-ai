...
- **想让回答带上网上的资料**：到"设置 → 联网搜索"启用一个搜索服务。默认那两个（Bing、DuckDuckGo）**不用注册、不用填 Key**，选中授权就能用；想更稳的话也可以用你自己的 SearXNG / Tavily / 博查。启用后问答输入框下面会多一个"联网搜索"开关。它只把你的**搜索词**发给搜索服务，文章正文不会发给它；用了网络资料时，回答会标明"根据网络资料"并附上链接。
  - 搜索服务和你的 DeepSeek 账号是分开的两件事——换任何模型都不影响它。
  - 免 Key 的那两个是直接读对方的搜索结果页，所以**对方改版时可能失效**；真失效了，回答会照实说"这次只依据文章本身"，不会拿别的东西充数。
...
- **联网搜索开启时**：你的**搜索词**会发给你在设置里选的那个搜索服务（不是 DeepSeek，也不是我们），文章正文不会发给它。搜索服务怎么保存数据由它自己的规则决定。**默认的 Bing / DuckDuckGo 之所以不用注册，是因为它直接读对方的搜索结果页**——这条路径属于灰色地带，对方随时可能改版或限流。
